...
- [C2b] **Empirically decoded 2026-08-26 (node repro vs real pi-core 0.84.1):** `findCutPoint` cuts ONLY at user/assistant entries — NEVER toolResult. When trailing toolResults alone exhaust `keepRecentTokens` (20k), walk-back finds NO valid cut point ≥ last entry → cutIndex defaults to `cutPoints[0]` = boundaryStart → keep-everything → `messagesToSummarize`+`turnPrefixMessages` both empty → `undefined`. Repro matrix: multi-turn=OK(15 msgs); single giant assistant msg=OK via split-turn prefix(1); tool-loop w/ 2k-tok results=OK(65 prefix); tool-loop w/ 20k-tok trailing results=**undefined**. Trap = "turn ends with toolResult(s) ≥ keepRecentTokens". Upstream #6879 mid-turn fix does NOT cover this variant (same geometry mid-turn). Fix design for upstream: degenerate cut (firstKeptEntryIndex===boundaryStart ∧ ¬isSplitTurn ∧ non-empty window) → return compact-all preparation — pi-vcc's `firstKeptEntryId=""` sentinel + orphan-recovery (`buildOwnCut`) is the missing semantic, already battle-tested.
...
- Upstream issues: https://github.com/earendil-works/pi/issues/6879, /8175, /8651. #6879 deep-read (2026-08-26): 3 contributor patches on `shouldStopAfterTurn` boundary (young921221/PeaceMaker-best/bodhi36 rebased `fix/6879-mid-turn-compaction-v2`, no open PR); PR #7540 (0.84.0) = overflow→compact→continue; `after_provider_response` = network-level hook (pre-stream), NOT agent-loop boundary; `compact()` aborts active run then gates (L1367, verified). Bun homedir cache = bun#29244 OPEN (validates LSL).
